...
def search():
    q2 = q.get()
    query = "select part_number, description_x, location, qty from onhand_ingredients where part_number like '%"+q2+"%' or description_x like '%"+q2+"%'"
    cursor.execute(query)
    rows = cursor.fetchall()
    update(rows)
...
wrapper1 = LabelFrame(root, text="LIST", height=150)
wrapper2 = LabelFrame(root, text="SEARCH")
wrapper3 = LabelFrame(root, text="MODIFY DATA")
