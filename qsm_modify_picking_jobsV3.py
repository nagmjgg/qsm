...
def clear():
    query = "select pick_id, creation_date, deliver_date, creation_time, job_number, product_name, received_by, " \
            "delivered_by, delivered_flag, moved_flag, returned_flag, return_date, comments from picking_job"
...
query = "select picking_job_id, part_number, part_description, location, qty, part_lot from picking_data"
cursor.execute(query)
rows = cursor.fetchall()
update(rows)
