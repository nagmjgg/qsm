...
onhand_columns = [x.replace("Unnamed: 3","unit") for x in onhand_columns]
...
onhand_sorted=onhand.sort_values(by='part_number')
...
data_location.sort_values(by="part_number")


#*********** split Column > Tracking (expiration , lot)

# *** Create Expiration column 
data['expiration'] = ""

# *** Create lot column 
data['lot'] = ""

#Extract text to fill columns 
...
onhand_available = onhand_available.sort_values(by='part_number')
#onhand.describe()
...
#exportar archivo 
try:
    file_name = folder+'onhand_out_'+actual_date()+'.csv'
...
inventory_availability_columns = inventory_availability.columns
...
#exportar archivo 
try:
    file_name = folder+'inventory_availability_'+actual_date()+'.csv'
...
#Normalize columns
inventory_availability_columns = [x.replace(" ","_") for x in inventory_availability_columns]
inventory_availability_columns = [x.lower() for x in inventory_availability_columns]

inventory_availability.columns = inventory_availability_columns
...
merged_inventory = pd.merge(left=data_inventory, right=data_onhand, left_on="part", right_on="part_number")

#exportar archivo
try:
    file_name = folder+'Combined_Inventory_Onhand_'+actual_date()+'.csv'
    merged_inventory.to_csv(file_name, index_label='id')
...
#**********************************************
#<<<<    Elements file export >>>>>>>>>
#**********************************************
print("")
print('***** Elements file export ******') 


#elements_qsm = merged_inventory[['part_number','description_x', 'unit', 'lot', 'expiration', 'available_x']]
elements_qsm = merged_inventory[['part_number','description_x', 'unit']]
elements_qsm = elements_qsm.drop_duplicates(["part_number","description_x"])
elements_qsm.columns = ['part_number','part_name','part_unit']

try:
    file_name = folder+'elements_qsm_'+actual_date()+'.csv'
    elements_qsm.to_csv(file_name, index_label='part_index')
    elements_qsm.columns = ['part_index','part_index','part_number', 'part_name', 'part_unit']
