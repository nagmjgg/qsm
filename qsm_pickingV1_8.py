...
query = "select pick_id, location, part_number, part_description, part_lot, expiration, qty from picking_data " \
...
pick_id_ent_job = Entry(wrapper0, textvariable=pick_id_job, state="disabled")    # state = "normal", "disabled", or "readonly"
...
save_pick_job_btn = Button(wrapper0, text="Save", command=save_pick_job)
save_pick_job_btn.grid(row=2, column=7, padx=PADX, pady=PADY, sticky=W)

new_pick_job_btn = Button(wrapper0, text="New")
new_pick_job_btn.grid(row=3, column=7, padx=PADX, pady=PADY, sticky=W)

modify_picking_data_btn = Button(wrapper0, text="Modify", command=modify_picking_data)
