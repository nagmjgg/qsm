...
def extract_text_to_new_column(dataframe, old_column, new_column, text_to_find,
                               text_range_to_extract_initial,text_range_to_extract_final):
    ...
    for i in dataframe.index:
        ...
        if not pd.isna(dataframe[old_column][i]):
            if text_to_find in dataframe[old_column][i]:
                ...
                dataframe[new_column][i] = dataframe[old_column][i][text_range_to_extract_initial:text_range_to_extract_final]
                #print(f"expiration : {data['tracking'][i][9:19]}")
            else:
                pass


extract_text_to_new_column(data,'tracking','lot','Lot',27,47)
extract_text_to_new_column(data,'tracking','expiration','ExpDate',9,19)
...
merge_inner = pd.merge(left=data_onhand, right=data_inventory, left_on="part_number", right_on="part")
