...
V3.3 validation is working for qty and unit
...
def add_new_element():
    ...
    cursor.execute(query, (date_sel, time_sel, part_number_sel.upper(), part_name_sel.upper(), part_lot_sel.upper(),
                           qty_sel, unit_sel,received_by_sel.upper(), delivered_by_sel.upper(), relocated_sel))
...
def modify_supply_record():
    subprocess.call("qsm_modify_supply_record V3_1.py", shell=True)
...
wrapper3 = LabelFrame(root, text="", font=font_wrap_title)


wrapper0.grid(row=1, column=0, padx=PADX, pady=PADY, sticky=EW)
wrapper1.grid(row=2, column=0, padx=PADX, pady=PADY, sticky=EW)
wrapper2.grid(row=3, column=0, padx=PADX, pady=PADY, sticky=EW)
wrapper3.grid(row=0, column=0, padx=PADX, pady=PADY, sticky=EW)


title_label = Label(wrapper3, text="WAREHOUSE SUPPLY RECORD", font=font_main_title)
title_label.pack()
...
qty_ent = Entry(wrapper2, textvariable=qty_sel, font=font_entry)
qty_ent.grid(row=5, column=1, padx=PADX, pady=PADY, sticky=W)
qty_ent.bind('<Tab>',lambda e: validate_if_empty_text(qty_ent))
...
unit_ent.insert(0,"ea")
unit_ent.bind('<Tab>',lambda e: validate_if_empty_numeric(unit_ent))
...
delivered_by_ent = AutocompleteCombobox(wrapper2, completevalues=list_warehouse, textvariable=delivered_by_sel, font=font_entry)
...
relocated_ent = AutocompleteCombobox(wrapper2, completevalues=list_relocated, textvariable=relocated_ent, font=font_entry)
...
view_btn = Button(wrapper2, text="View", command=modify_supply_record, font=font_button, width=10)
view_btn.grid(row=10, column=2, padx=5, pady=3, sticky=W)
