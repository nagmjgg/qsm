...
try:
    onhand = pd.read_csv(initial_filename, encoding='latin1',header=3)
except: 
...
'''
for i in onhand_available.index:
    #print(i)
    #print(data["Location"][i])
    if onhand_available["Part Number"][i] and onhand_available["Part Number"][i+1]:
        onhand_available["data_available"][i] = onhand_available["Location"][i] + "(" + onhand_available["Qty"][i]
    else:
        data["available"][i]=1
'''

#****** create column expired *******
data_expired = data[data["Location"]!='Stock Bin In Process']
data_expired.sort_values(by=["Part Number","Location"])
data["expired"] = 0
data["expired_flag"] = 0
...
for i in data.index:
    #print(i)
    #print(data["Location"][i])
    if data["Location"][i][0:5] in string:
        data["expired"][i]=data["Qty"][i]
        data["expired_flag"][i]=1
    else:
        data["expired_flag"][i]=0
print("Expired ok")
...
# ******** Exportar archivo  *********
try:
    file_name = folder+'OnHand_'+str(date_final_file)+'.csv'
